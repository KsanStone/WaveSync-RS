...
impl Inner {
    ...
    pub fn handle_samples(&self, samples: Vec<Vec<f32>>) {
        ...
        drop(buffer);
        self.samples_written.fetch_add(samples[0].len() as u64, Ordering::Release);
    ...
    fn do_fft(&self, channels: usize) {
        ...
        for _ in 0..to_do {
            ...
            for i in 0..channels {
                ...
                let end_idx = audio_buffer[i].len().saturating_sub(offset_from_buffer_end as usize);
                let start_idx = end_idx.saturating_sub(fft_plan.len());
                if end_idx - start_idx != fft_plan.len() {
                    debug!("Buffer too smol {} {} {}", end_idx, start_idx, end_idx - start_idx);
                    continue
                } else {
                    debug!("Buffer ok {} {} {}", end_idx, start_idx, end_idx - start_idx);
                }

                let sample_iter = audio_buffer[i].range(start_idx..end_idx);
                let mut fft_in = sample_iter.map(|sample| Complex::new(*sample, 0.0f32)).collect::<Vec<_>>();
