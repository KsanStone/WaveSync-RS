...
impl AudioBackend for DummyAudioBackend {
    ...
    fn start_capture(&mut self, _source: CaptureSource) {
        ...
        for i in 0 .. 48000 {
            frame[i] = (self.sequence_index.fetch_add(1, Ordering::Relaxed) as f32 / 8000.0 * std::f32::consts::PI * 2.0).sin()
        }
            ...
            loop {
                if let Some(callback) = callback_weak.upgrade() {
                    ...
                    for i in 0 .. frame.len() {
                        frame[i] = (sequence_index.fetch_add(1, Ordering::Relaxed) as f32 / 8000.0 * std::f32::consts::PI * 2.0).sin()
                    }
