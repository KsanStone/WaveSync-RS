...
impl Default for PlotData {
    fn default() -> Self {
        Self {
            x_axis: Some(Axis { min: 0.0, max: 100.0, logarithmic: false }),
            x_axis_grid: true,
            y_axis: Some(Axis { min: 0.0, max: 100.0, logarithmic: false }),
            y_axis_grid: true,
        }
    }
}
...
impl <'a> GpuPlot<'a> {
    pub fn new(data: &'a mut PlotData) -> Self {
        Self { plot_data: data, grid_color: Color32::from_gray(50), label_color: Color32::from_gray(200) }
    }
    ...
    pub fn show(self, ui: &mut Ui, paint_callback: impl CallbackTrait + 'static) -> Response {
        ...
        let content_rect = rect.sub(Margin {
            left: if self.plot_data.y_axis.is_some() { AXIS_WIDTH } else { 0 },
            right: 0,
            top: 0,
            bottom: if self.plot_data.x_axis.is_some() { AXIS_WIDTH } else { 0 },
        });
        ...
        if let Some(axis) = self.plot_data.x_axis.as_ref() {
            for pos in axis.tick_positions(content_rect.width()) {
                ...
                painter.line_segment([Pos2::new(px_pos, content_rect.min.y), Pos2::new(px_pos, content_rect.max.y)], (1.0, self.grid_color));
                painter.text(Pos2::new(px_pos, content_rect.max.y + 2.0), Align2::CENTER_TOP, format_significant(pos, PLOT_DIGITS), FontId::default(), self.label_color);
            }
        }
        ...
        if let Some(axis) = self.plot_data.y_axis.as_ref() {
            for pos in axis.tick_positions(content_rect.height()) {
                ...
                painter.line_segment([Pos2::new(content_rect.min.x, px_pos), Pos2::new(content_rect.max.x, px_pos)], (1.0, self.grid_color));
                painter.text(Pos2::new(rect.min.x + 2.0, px_pos), Align2::LEFT_CENTER, format_significant(pos, PLOT_DIGITS), FontId::default(), self.label_color);
            }
        }

        ui.painter().add(egui_wgpu::Callback::new_paint_callback(content_rect, paint_callback));
