...
pub trait Visualizer {
    ...
    fn get_draw_callback(&self, rect: Rect) -> epaint::PaintCallback;
...
impl <'a> VisualizerWidget<'a> {
    pub fn new(visualizer: Box<dyn Visualizer>, ctx: &'a egui::Context) -> Self {
        Self { visualizer, ctx }
    }
}
...
impl <'a> Widget for VisualizerWidget<'a> {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        ui.painter()
            .add(self.visualizer.get_draw_callback(content_rect));
...
macro_rules! impl_settings {
    ($name:expr, $ui:ident, $this:ident, $draw_callback:expr) => {
                ...
                egui::Window::new($name)
                    ...
                    .show(ctx, |ui| {
                        ...
                        if ui.button("Close").clicked() {
                            self.settings_open.store(false, std::sync::atomic::Ordering::Release);
                        }
                    });
            }
        }
        ...
        fn open_settings(&self) {
            self.settings_open.store(true, std::sync::atomic::Ordering::Release);
        }
    }
}
